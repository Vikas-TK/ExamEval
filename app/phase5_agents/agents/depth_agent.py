    ...
    try:
        ...
        if openai_client:
            prompt = (
                ...
                "STRICT RULE: Judge depth purely by the AMOUNT and STRUCTURE of reasoning — number of "
                "explanatory steps, cause-and-effect chains, sub-arguments, examples — relative to the "
                "Expected Depth Level. Do NOT lower the score because a statement is factually wrong; "
                "a well-elaborated but incorrect explanation still scores high on depth. Only lower the "
                "score because the explanation is too brief, one-line, or lacks structure. The words "
                "'incorrect', 'wrong', 'misconception', 'inaccurate', 'error', and 'mistake' are BANNED "
                "from weak_sections and remarks — rephrase every weakness purely in terms of missing "
                "elaboration/structure (e.g. 'does not explain why this step happens', 'single sentence "
                "with no supporting reasoning'), never in terms of correctness.\n\n"
